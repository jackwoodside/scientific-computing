...
md"""
## Eigenstates of an Arbitrary 1D Potential

The time-independent Schrödinger equation, in the position basis, is given by

$-\frac{\hbar^2}{2m}\frac{d^2\psi}{dx^2} + V(x)\psi = E\psi$

where $\psi$ is the wavefunction, $E$ is the energy, $V(x)$ is the potential, and $m$ is the mass. We'll consider a particle confined to some interval $[0,L]$, which gives the boundary conditions

$\psi(0) = \psi(L) = 0.$

This differential equation has a set of eigenfunctions $\psi_n$, with corresponding eigenenergies $E_n$, which form a basis for the solutions to the equation. A simple example would be in the infinite square well where the potential is $V(x) = 0$, which has the analytic solution
