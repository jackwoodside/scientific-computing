...
md"""
## Carrier States
For some diseases, a proportion of infected individuals $$q$$ can enter a carrier state before recovery, where they have no symptoms but are still infectious. They have a lower transmission rate $$\varepsilon\beta$$ where $$0 < \varepsilon < 1$$, and have an average carrier period of $$1/\Gamma$$. The system of differential equations is then

$$\frac{dS}{dt} = -\beta S(I+\varepsilon C), \quad \frac{dI}{dt} = \beta S(I+\varepsilon C) -\gamma I,$$
$$\frac{dC}{dt} = q\gamma I - \Gamma C, \quad \frac{dR}{dt} = (1-q)\gamma I + \Gamma C.$$
"""
...
function sicr!(dn,n,p,t)
	S, I, C, R = n
	β, γ, Γ, ε, q = p
	dn[1] = dS = -β*S*(I+ε*C)
	dn[2] = dI = β*S*(I+ε*C) - γ*I
	dn[3] = dC = q*γ*I - Γ*C
	dn[4] = dR = (1-q)*γ*I + Γ*C
end
...
@bind avgPeriod Slider(1:10; default=6, show_value=true)

# ╔═╡ 2527aed4-cbaa-4803-927e-d1f1df7ee656
# Average carrier period 1/Γ
@bind avgCarrier Slider(1:10; default=8, show_value=true)

# ╔═╡ 99e3b86b-f7aa-4961-9798-4c7230fcf99a
# Carrier infectivity ε
@bind ε Slider(0 : 0.05 : 1; default=0.5, show_value=true)

# ╔═╡ a285df96-fa2f-48aa-97c8-15dce020467b
# Carrier proportion q
@bind q Slider(0 : 0.1 : 1; default=0.4, show_value=true)
...
begin
	# Initial conditions
	p = [β, 1/avgPeriod, 1/avgCarrier, ε, q] # β, γ, Γ, ε, q
	n0 = [1 - infected/100, infected/100, 0, 0] # S0, I0, C0, R0
	
	tspan = (0.0, 80.0)
	prob = ODEProblem(sicr!,n0,tspan,p)
	sol = solve(prob,Tsit5())
	plot(sol, lw = 2, label = ["S" "I" "C" "R"])
	plot!(title = "Carrier States", xlabel = "Days", ylabel = "Proportion")
end
