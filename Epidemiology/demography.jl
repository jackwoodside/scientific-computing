...
md"""
## Demography
The basic model assumes a constant population with no births and deaths. We will now add births and deaths at a balanced rate, so the total population will remain fixed, and there will be deaths from all three classes, with a birth/death rate of $$\mu$$. The system of differential equations is then
...
function sird!(dn,n,p,t)
	S, I, R = n
	μ, β, γ = p
	dn[1] = dS = μ - β*S*I - μ*S
	dn[2] = dE = β*S*I - γ*I - μ*I
	dn[3] = dR = γ*I - μ*R
end

# ╔═╡ be96345b-8678-43b4-874b-d9dedfcdf595
# Birth/death rate μ
@bind μ Slider(0.1 : 0.05 : 1; default=0.5, show_value=true)
...
begin
	# Initial conditions
	p = [μ, β, 1/avgPeriod] # μ, β, γ
	n0 = [1 - infected/100, infected/100, 0] # S0, I0, R0
	
	tspan = (0.0, 80.0)
	prob = ODEProblem(sird!,n0,tspan,p)
	sol = solve(prob,Tsit5())
	plot(sol, lw = 2, label = ["S" "I" "R"])
	plot!(title = "SIR w/ Demography", xlabel = "Days", ylabel = "Proportion")
end
